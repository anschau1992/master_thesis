...
## !!!!!!!!!! HACKED VERSION - TODO: INCLUDE PARAM IN 'compare-random.sh' to choose filters
...
LINE_LENGTH=$(wc -l < ../../data/only_noun/test.src.de)
...
do
    src_de=$(sed "${i}!d" "../../data/only_noun/test.src.de")
    test_src_de+=( "$( [[ -z "$src_de" ]] && echo "[NULL]" || echo ${src_de} )" )

    trg_de=$(sed "${i}!d" "../../data/only_noun/test.trg.de")
    test_trg_de+=( "$( [[ -z "$trg_de" ]] && echo "[NULL]" || echo ${trg_de} )" )

    trg_de_output=$(sed "${i}!d" "../../data/only_noun/test.trg.de.output")
    test_trg_de_output+=( "$( [[ -z "$trg_de_output" ]] && echo "[NULL]" || echo ${trg_de_output} )" )

    src_en=$(sed "${i}!d" "../../data/only_noun/test.src.en")
    test_src_en+=( "$( [[ -z "$src_en" ]] && echo "[NULL]" || echo ${src_en} )" )

    base_de=$(sed "${i}!d" "../../data/only_noun/test.base.de")
    test_base_de+=( "$( [[ -z "$base_de" ]] && echo "[NULL]" || echo ${base_de} )" )

    scoring_line=$(sed "${i}!d" "../../data/only-noun-scoring.output")
